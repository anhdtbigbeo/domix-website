...
https.get({
  hostname: 'github.com',
  path: '/anhdtbigbeo/domix-website/settings/pages',
  headers: {
  ...
  res.on('end', () => {
    fs.writeFileSync('pages_settings.html', body);
    const forms = body.match(/<form[^>]+>/gi) || [];
    console.log("Forms found count:", forms.length);
    forms.forEach((f, i) => console.log(`Form [${i}]: ${f}`));

    const inputs = body.match(/<input[^>]+>/gi) || [];
    console.log("Inputs count:", inputs.length);
    inputs.filter(inTag => inTag.includes('token') || inTag.includes('branch')).forEach(inTag => console.log("Input:", inTag));
  });
});
