...
async function test() {
  console.log("Testing GET /anhdtbigbeo...");
  const res1 = await get('/anhdtbigbeo');
  console.log("Status:", res1.status);
  
  // Look for any input or token in profile page
  const tokenMatch = res1.body.match(/name="authenticity_token"\s+value="([^"]+)"/i) || res1.body.match(/value="([^"]+)"\s+name="authenticity_token"/i);
  if (tokenMatch) {
    console.log("Found authenticity_token on profile page:", tokenMatch[1]);
  } else {
    console.log("No authenticity_token on profile page.");
  }

  // Check new repo JSON payload endpoint
  const res2 = await get('/new', { 'Accept': 'application/json' });
  console.log("GET /new with JSON status:", res2.status, res2.headers['content-type']);
  console.log("Body preview:", res2.body.slice(0, 200));
}
