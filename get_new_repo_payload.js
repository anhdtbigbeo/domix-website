...
async function main() {
  const res = await get('/new', { 'Accept': 'application/json' });
  const data = JSON.parse(res.body);
  console.log("JSON Payload Keys:", Object.keys(data));
  console.log("Payload content keys:", Object.keys(data.payload || {}));
  console.log("Full payload string preview:", JSON.stringify(data.payload).slice(0, 1000));
}
